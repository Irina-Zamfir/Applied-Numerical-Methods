function [classes] = predict_classes(X, weights, ...
  ...
  theta_end = hidden_layer_size * (input_layer_size + 1);
  Theta1 = reshape(weights(1: theta_end), hidden_layer_size, (input_layer_size + 1));
  Theta2 = reshape(weights(theta_end + 1 :end), output_layer_size, (hidden_layer_size + 1));
  ...
  % Predict the class as the index of the max output activation 
    % returns m x 1 vector of predictions 
  [~, classes] = max(a3, [], 2);
